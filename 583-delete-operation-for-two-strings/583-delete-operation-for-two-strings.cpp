    ...
    int minDistance(string word1, string word2) {
        ...
        for(int i = 0; i <= size(word1); i++) 
         for(int j = 0; j <= size(word2); j++) 
          if(!i || !j) dp[i][j] = i + j; // if one word == "", 
                                                //all other characters of other word need to be deleted
                // if characters match  ?  dont delete     : 1 deletion +
                                                    //min to equalize after deletion of (word1[i], word2[j])
          else 
